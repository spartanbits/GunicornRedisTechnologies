...
sub plot_stats {
    ...
    select GP;
    foreach my $output_ext ($imgfmt, "tn.png") {
        if (($output_ext eq "png") || ($output_ext eq "tn.png")) {
            $output_type = "png";
        } elsif ($output_ext eq "ps") {
            $output_type = "postscript color ";
        } elsif ($output_ext eq "pdf") {
            $output_type = "pdf color ";
        } elsif ($output_ext eq "svg") {
            $output_type = "svg";
        }
        # gnuplot styles and options
        print "set size $thumbnail_size,$thumbnail_size\n" if (($output_ext eq "tn.png") && $oldgnuplot);
        print "set xtics rotate by +90\n"  if ((($output_ext eq "png") || ($output_ext eq "tn.png")) and ($rotate_xtics));
        print "set style data  $style\n";
        if (($output_ext eq "tn.png") and not $oldgnuplot) {
            print "set terminal $output_type tiny size 320,240\n";
        } elsif ((($output_type eq "svg") or (($output_type eq "png"))) and not $oldgnuplot) {
        ...
        foreach $d (0..$#{$title}) {
            # gnuplot headings
            if ($output_ext eq "tn.png") {
                print "set output \"$imgdir/graphes-$datatype-@{$title}[$d]_$output_ext\"\n";
            } else  {
...
sub parse_stats_file {
  ...
  my @col = ("rate","mean","stdvar","max_sample","min_sample"); #session, perfs et transaction
